...
/**
 * Test the slboostnavbar file
 *
 * @package    theme_slboost
 * @covers     \theme_slboost\slboostnavbar
 * @copyright  2021 Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class slboostnavbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected): void {
        ...
        $slboostnavbar = $this->getMockBuilder(slboostnavbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(slboostnavbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setValue($slboostnavbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->invoke($slboostnavbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($slboostnavbar);
        $actual = [];
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->invoke($slboostnavbar, $navigationmenu);
